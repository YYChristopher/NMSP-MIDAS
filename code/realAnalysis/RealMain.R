...
D <- as.matrix(cbind(fredqd$`S&P 500` * 100, (fredqd$GS10 - fredqd$TB3MS) * 20, fredqd$ADS * 10)) 
D <- D[-(1:notuseindex), ]
NH <- ncol(D)
...
It$nsave = 10000 # total save sample, can change
It$nburn = 10000 # burn sample, can change
It$nthin = 1
...
imp1 <- which(abs(beta.real.estimation[1,])>0.1)
imp2 <- which(abs(beta.real.estimation[2,])>0.1)

tran.est <- tran(zeta.estimation, phi.estimation, d, NS)
p_tran.est <- tran_p(tran.est, NS)
ptran.estimation <- apply(p_tran.est, c(3, 4), mean)
...
plot.gdp.growth.color <- ggplot() +
  ...
  theme_minimal() +
  theme(
    axis.title.x = element_text(size = 16),  # x title
    axis.title.y = element_text(size = 16),  # y title
...
ggsave("./code/realAnalysis/Plots/NMSP-MIDAS.png", plot = plot.gdp.growth.color, width = 8, height = 6, dpi = 300)
...
# Forecast the 1st sample in test set
plast_prob <- temp.mat[nrow(temp.mat), ] / sum(temp.mat[nrow(temp.mat), ])
tran_test_est <- tran_acc(zeta.estimation, phi.estimation, d, NS)
