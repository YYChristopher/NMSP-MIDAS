...
for (r in 1:Rep) {
  ...
  for (n in 1:N) {
    D_test_mat[((n-1)*NT+1):(n*NT), ] <- matrix(unlist(data_list[[n]]), ncol = total_dim, byrow = FALSE) # 注意要先将list数据Unlist再填入矩�?
  }
  ...
  parameters_path <- "./code/simulation/Paras_estimation"
  ...
  phi_est <- as.numeric(parsed_data[[5]])
  zeta_est <- matrix(as.numeric(parsed_data[[6]]), nrow = NS, ncol = NS)
  plast_prob <- as.numeric(parsed_data[[7]])
  ...
  for (tr in 1:trial_count) {
    ...
    tran_test_est <- tran_acc(zeta_est, phi_est, h_test, NS)
    # Calculating transition probabilities
    tran_p_test_est <- tran_p(tran_test_est, NS)
    # Calculate state sequence
    state_test_est <- tran_state(tran_p_test_est, p0_est)
...
for (r in 1:Rep) {
  ...
  writeLines(unlist(char_est_list), paste0("./code/simulation/Forecast_results", "/", "forecast_res", r, ".txt"))
}