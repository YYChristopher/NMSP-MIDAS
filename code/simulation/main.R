...
sourceCpp("./code/simulation/update.cpp")
...
NH = 3 # number of external variables in transition model
Rep = 1
...
all_phi <- array(0, dim = c(Rep, NH))
all_zeta <- array(0, dim = c(Rep, NS, NS))
...
for (r in 1:Rep) {
  ...
  for (n in 1:N) {
    D_test_mat[((n-1)*NT+1):(n*NT_oos), ] <- matrix(unlist(data_list.test[[n]]), ncol = total_dim, byrow = FALSE) # 注意要先将list数据Unlist再填入矩�?
  }
  ...
  lop <- runif(NS - 1) 
  phi <- matrix(rnorm(NH), nrow = 1, ncol = NH, byrow = TRUE) 
  zeta <- matrix(rnorm(NS * NS), nrow = NS, ncol = NS)
  zeta[, NS] <- 0
  ...
  repeat{
    tryCatch({
      ...
      start.time = Sys.time()
      result = mcmc(y_cpp, x_cpp, fc_cpp, d_cpp, alpha_cpp,
                    sigma_cpp, lop_cpp, zeta_cpp, phi_cpp, 
                    betan_cpp, lambda2_cpp, tau2_cpp, pi0_cpp, 
                    pi1_cpp, Z_cpp, sg, groups, s_cpp, priorbeta, 
                    N, NT, NS, NF, NH, c_lop, c_zeta, c_phi, iter, rep, ntot, ndraw, ng, eg)
      end.time = Sys.time()
      
      # Result Analysis
      tau_result = array(result$tau_str, dim = c(NS - 1, ndraw))
      sigma_result = array(result$sigma_str, dim = c(NS, ndraw))
      phi_result = array(result$phi_str, dim = c(NH, ndraw))
      ...
      zeta_result = array(0, dim = c(ndraw, NS, NS))
      for(i in 1:NS){
        for(j in 1:NS){
          zeta_result[ ,i ,j] = result$zeta_str[(0:(ndraw - 1)) * NS * NS + (i - 1) * NS + j]
        }
      }
      ...
      ## posterior mean
      tau.estimation = apply(tau_result, 1, mean)
      zeta.estimation = apply(zeta_result, c(2:3), mean)
      phi.estimation = apply(phi_result, 1, mean)
      ...
      tau.sd = apply(tau_result, 1, sd)
      zeta.sd = apply(zeta_result, c(2:3), sd)
      phi.sd = apply(phi_result, 1, sd)
      ...
      all_phi[r, ] <- phi.estimation
      all_zeta[r, , ] <- zeta.estimation
      ...
      paras_est_list <- list(beta_est = all_beta[r, , ], beta_org_est = all_alpha[r, , ],
                             intercept_est = all_intercept[r, ], sigma_est = all_sigma[r, ], phi_est = all_phi[r, ], 
                             zeta_est = all_zeta[r, , ], last_prob = plast_prob)
      char_est_list <- lapply(paras_est_list, function(x) paste(x, collapse = ", "))
      writeLines(unlist(char_est_list), paste0("./code/simulation/Paras_estimation", "/", "Paras_est_rep", specific_number, ".txt"))
...
# ############### Forecast with expanding window ##############
# # Standalization for test data. 
# x.test = array(0, dim = c(N, NT_oos, NF))
# y.test = array(0, dim = c(NT_oos, N))
# d.test = array(0, dim = c(N, NT_oos, NH))
# for (n in 1:N) {
#   # result_x <- standardize_me2(X_test_alm_mat)
#   result_x <- standardize_me(X_test_alm_mat)
#   result_y <- as.matrix(Y_test_mat)
#   x_scale = result_x$y
#   mu_x = result_x$mu
#   sig_x = result_x$sig
#   y_scale = result_y
...
# # tran_p_1 <- tran_p_test_est[1, nt, , ]
# tran_p_1 <- tran_p_test_est[1, nt + 1, , ]
