...
Almon_lagQ <- function(polydegree, C) {
  Q <- matrix(0, nrow = polydegree + 1, ncol = C)
  for (ii in 0:polydegree) {
    Q[ii + 1, ] <- (1:C)^ii
  }
...
p0_gen <- function(tau) {
  # transition probability model calculate p0
  NS <- length(tau) + 1
  p0 <- rep(1, NS)
...
# tran_acc3 <- function(zeta, phi, d, NS) {
#   N <- dim(d)[1]
#   NT <- dim(d)[2]
#   
#   tran <- array(0, dim = c(N, NT, NS, NS))
...
#   for (n in 1:N) {
#     for (t in 1:NT) {
#       phid[n, t, , ] = matrix(d[n, t, ] %*% t(phi), nrow = NS, ncol = NS, byrow = TRUE)
#     }
#   }
#   
#   for (u in 1:NS) {
...
tran_state <- function(tran_p, p0) {
  ...
  # give value to first time in state
  for (i in 1:N) {
...
# tran_state_each <- function(tran_p, initial_state, N) {
#   # get dimension of tran_p
#   NS <- dim(tran_p)[2]
#   
#   # initialize state matrix
#   state <- as.vector(rep(0, N))
#   state[1] <- initial_state
#   
#   # give value to state
#   for (t in 2:N) {
#     prev_state <- state[t - 1]
#     state[t] <- sample(1:NS, 1, prob = tran_p[prev_state, ])
#     # state[i, t] <- which.max(tran_p[i, t, prev_state, ])
#   }
#   
#   return(state)
...
# Evaluations for hidden states decision
...
# Evaluation for parameter estimations
PE_evaluation <- function(estimates, true){
  
  mse <- mean((estimates - true)^2)
  bias <- mean(estimates) - true[1]
  variance <- var(estimates)
  return(list(mse = mse, bias = bias, variance = variance))
}


PEvec_evaluation <- function(estimates, true){
  
  mse <- mean((estimates - true)^2)
  bias <- mean(estimates) - mean(true)
  variance <- var(estimates)
  return(list(mse = mse, bias = bias, variance = variance))
}
...
mse_vector <- function(y, y_hat) {
  ...
  total_mse <- mean(rowSums(squared_errors))
...
tpr.fpr.mcc <- function(beta_real, all.beta.est.median, Rep) {
  ...
  for (r in 1:Rep) {
    ...
    # initialization
    TP <- sum(beta_real == 0 & beta_est == 0)  # true positive
    FP <- sum(beta_real == 0 & beta_est != 0)  # false positive
    FN <- sum(beta_real != 0 & beta_est == 0)  # false negative
    TN <- sum(beta_real != 0 & beta_est != 0)  # true negative
...
dmw.test.myself <- function(e1, e2){
  ...
  d_bar <- mean(d)
  T <- length(d)
  s_d <- sqrt(var(d) / T)  # standard deviation
  dmw_stat <- d_bar / s_d
