...
// [[Rcpp::export]]
NumericVector f_sigma2(NumericVector y, NumericVector Bbeta, NumericVector betan, NumericVector tau2,
                      NumericVector Z, NumericVector sg, NumericVector s, int N, int NT, int NS, int ng) {
  NumericVector sigma_out(NS);
  NumericVector sigma_sum(NS);
  NumericVector Z_used(ng);
...
List mcmc(NumericVector y, NumericVector x, NumericVector fc, NumericVector d, NumericVector alpha,
  ...
  NumericVector tau2_str(S*NS*ng);
  NumericVector Z_str(S*NS*ng);
  NumericVector betan_str(S*NS*ng);
  NumericVector lambda2_str(S*NS*ng);
  NumericVector pi0_str(S*NS);
    ...
    if(t>(T-S-1)){
      ...
      for(int i=0; i<NS; ++i){
        for(int j=0; j<ng; ++j){
          ...
          tau2_str[ind*NS*ng+i*ng+j] = tau2[j*NS + i];
          Z_str[ind*NS*ng+i*ng+j] = Z[j*NS + i];
          betan_str[ind*NS*ng+i*ng+j] = betan[j*NS + i];
        }
      }
      for(int i=0; i<NS; ++i){
        sigma_str[ind*NS+i] = sigma[i];
        pi0_str[ind*NS+i] = pi0[i];
      }
    }
  ...
  return List::create(Rcpp::Named("tau_str") = tau_str,
                      ...
                      Rcpp::Named("pi0_str") = pi0_str,
                      Rcpp::Named("tau2_str") = tau2_str,
                      Rcpp::Named("Z_str") = Z_str,
                      Rcpp::Named("lambda2_str") = lambda2_str,
                      Rcpp::Named("betan_str") = betan_str,
